...
def do_load_flow(ps_sim):
    ...
    sl_bus_ids = [i for i, bus in enumerate(ps_sim.busses) if bus.lf_type == 'SL'][0]
    ...
    for ia in count(0):
        ...
        for i, idx1 in enumerate(pv_and_pq_bus_ids):
            ...
            for k, idx2 in enumerate(pv_and_pq_bus_ids):
                if i == k:
                    ...
                    J[:, i, k] = (-abs(v_bus[:, idx1]) * abs(v_bus[:, idx2]) * (y_bus[:, idx1, idx2].real * torch.sin(
                        torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])) - y_bus[:, idx1,
                                                                                     idx2].imag * torch.cos(
                        torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])))).squeeze()

            # dP/dV
            for k, idx2 in enumerate(pq_bus_ids):
                if i == k + len(pv_and_pq_bus_ids):
                    J[:, i, k + len(pv_and_pq_bus_ids)] = -(
                                abs(v_bus[:, idx1]) * y_bus[:, idx1, idx2].real - s_calc[:, idx1].real / abs(
                            v_bus[:, idx1])).squeeze()
                else:
                    J[:, i, k + len(pv_and_pq_bus_ids)] = (
                                abs(v_bus[:, idx1]) * abs(v_bus[:, idx2]) * (y_bus[:, idx1, idx2].real * torch.cos(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])) + y_bus[:, idx1,
                                                                                         idx2].imag * torch.sin(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])))).squeeze()
        ...
        for i, idx1 in enumerate(pq_bus_ids):
            # dQ/dTheta
            for k, idx2 in enumerate(pv_and_pq_bus_ids):
                if i + len(pv_and_pq_bus_ids) == k:
                    J[:, i + len(pv_and_pq_bus_ids), k] = (
                                abs(v_bus[:, idx1]) ** 2 * y_bus[:, idx1, idx2].real - s_calc[:, idx1].real).squeeze()
                else:
                    J[:, i + len(pv_and_pq_bus_ids), k] = (
                                -abs(v_bus[:, idx1]) * (y_bus[:, idx1, idx2].real * torch.cos(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])) + y_bus[:, idx1,
                                                                                         idx2].imag * torch.sin(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])))).squeeze()
            # dQ/dV
            for k, idx2 in enumerate(pq_bus_ids):
                if i + len(pv_and_pq_bus_ids) == k + len(pv_and_pq_bus_ids):
                    J[:, i + len(pv_and_pq_bus_ids), k + len(pv_and_pq_bus_ids)] = (
                                abs(v_bus[:, idx1]) * y_bus[:, idx1, idx2].imag - s_calc[:, idx1].imag / abs(
                            v_bus[:, idx1])).squeeze()
                else:
                    J[:, i + len(pv_and_pq_bus_ids), k + len(pv_and_pq_bus_ids)] = (
                                -abs(v_bus[:, idx1]) * (y_bus[:, idx1, idx2].real * torch.sin(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])) - y_bus[:, idx1,
                                                                                         idx2].imag * torch.cos(
                            torch.angle(v_bus[:, idx1]) - torch.angle(v_bus[:, idx2])))).squeeze()
        ...
        if torch.max(torch.abs(error)) < 1e-7:
